        ...
        if self.num_mamba_like_layers > 0:
            ...
            for group_idx in range(len(self.input_batch.block_table.block_tables)):
                ...
                if group_idx in self._compact_gdn_group_ids:
                    for i, req_id in enumerate(contents.req_ids):
                        state_indices_cpu[i] = self._get_compact_gdn_private_state_index(req_id, group_idx)
                else:
            ...
            for group_idx in range(len(self.input_batch.block_table.block_tables)):
                if group_idx in self._compact_gdn_group_ids:
                    state_indices_cpu = torch.tensor([
                        self._get_compact_gdn_private_state_index(self.input_batch.req_ids[i], group_idx)
